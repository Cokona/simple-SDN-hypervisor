...
datadict = {2:{2:0,3:0}, 5:{2:0,3:0}, 3:{2:0,3:0},6:{2:0,3:0}}
datadict_compare = {2:{2:0,3:0}, 5:{2:0,3:0}, 3:{2:0,3:0},6:{2:0,3:0}}
...
class SimpleSwitch(app_manager.RyuApp):
	...
	def add_flow(self, datapath, in_port, dst, actions, priority=0, hard_timeout=0):
		...
		mod = datapath.ofproto_parser.OFPFlowMod(
			...
			command=ofproto.OFPFC_ADD, idle_timeout=0, hard_timeout=hard_timeout,
	...
	# Task3 EXTRA - add_flow code
	def block_traffic_DATA(self, datapath, in_port, actions=[], priority=0, hard_timeout=0):
		ofproto = datapath.ofproto
	...
	def _port_stats_reply_handler(self, ev):
		...
		datadict[ev.msg.datapath.id][stat.port_no] = stat.rx_bytes
		compare_value = datadict[ev.msg.datapath.id][stat.port_no] -  datadict_compare[ev.msg.datapath.id][stat.port_no] 
		if (ev.msg.datapath.id in students_sw_list) and compare_value >= 10**9:
			self.logger.info('BLOCKKKK from %d', ev.msg.datapath.id)
			datadict_compare[ev.msg.datapath.id][stat.port_no] = datadict[ev.msg.datapath.id][stat.port_no] 
			hard_timeout = 60 - self.counter_student
			self.block_traffic_DATA(ev.msg.datapath, stat.port_no, priority=10, hard_timeout=hard_timeout)
		elif  (ev.msg.datapath.id in professors_sw_list) and compare_value >= 5*(10**9):
			self.logger.info('BLOCKKKK from %d', ev.msg.datapath.id)
			datadict_compare[ev.msg.datapath.id][stat.port_no] = datadict[ev.msg.datapath.id][stat.port_no] 
			hard_timeout = 120 - self.counter_prof
			self.block_traffic_DATA(ev.msg.datapath, stat.port_no, priority=10, hard_timeout=hard_timeout)
	...
	def _monitor(self):
		...
		while True:
			...
			self.counter_prof += 5
			self.counter_student += 5
			if self.counter_prof == 120:
				self.logger.info('Reset 2-minute-counter for professors.')
				self.counter_prof = 0
				datadict_compare.update({2:datadict[2],5:datadict[5]})
			if self.counter_student == 60:
				self.logger.info('Reset 1-minute-counter for students.')
				self.counter_student= 0
				datadict_compare.update({3:datadict[3],6:datadict[6]})
				#self.logger.info(datadict_compare)

			for dp in self.datapaths.values():
				#Request stats
				if dp.id in professors_sw_list or dp.id in students_sw_list:
